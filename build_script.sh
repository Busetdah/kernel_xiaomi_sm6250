...
# DTB_2="${KERNEL_DIR}"/out/arch/arm64/boot/dts/qcom/cust-atoll-ab.dtb
...
LATEST_COMMIT="[$COMMIT_POINT](https://github.com/silont-project/kernel_xiaomi_sm6250/commit/$CHEAD)"
LOGS_URL="[See Drone CI Build Logs Here](https://cloud.drone.io/silont-project/kernel_xiaomi_sm6250/$DRONE_BUILD_NUMBER)"
...
makekernel() {
    ...
    echo "ThisIsTag@Indonesia" > "$KERNEL_DIR"/.builderdata
...
packingkernel() {
    ...
    if [[ "${DTB_TYPE}" =~ "single" ]]; then
        ...
        cp "${DTB_2}" "${ANYKERNEL}"/cust-atoll-ab.dtb
    fi
